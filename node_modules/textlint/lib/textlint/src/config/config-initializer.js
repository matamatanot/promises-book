...
var getTextlintDependencyNames = function (dir) {
    return readPkg(dir)
        ...
        return pkgNames.filter(function (pkgName) {
            var ruleOrFilter = pkgName.indexOf(types_1.TextlintPackageNamePrefix.filterRule) !== -1 ||
                pkgName.indexOf(types_1.TextlintPackageNamePrefix.rule) !== -1;
...
exports.createConfigFile = function (options) {
    ...
    return getTextlintDependencyNames(dir).then(function (pkgNames) {
        ...
        var filters = pkgNames
            ...
            return pkgName.indexOf(types_1.TextlintPackageNamePrefix.filterRule) !== -1;
        })
            .map(function (filterName) {
            return filterName.replace(types_1.TextlintPackageNamePrefix.filterRule, "");
        });
        var rules = pkgNames
            .filter(function (pkgName) {
            return pkgName.indexOf(types_1.TextlintPackageNamePrefix.rule) !== -1;
        })
            .map(function (filterName) {
            return filterName.replace(types_1.TextlintPackageNamePrefix.rule, "");
        });
